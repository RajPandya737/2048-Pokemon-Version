...
public class SteelBattle extends AppCompatActivity {
    MediaPlayer player;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_steel_battle);
    ...
    public void throwball(View view) {
        ...
        Timer timer;
        if (a != 1) {
            t.setText("You have caught the Steeleon!");
            battle.setImageResource(R.drawable.s7after);

            timer = new Timer();
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    Intent i = new Intent(SteelBattle.this, GameScreen.class);
            ...
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    Intent i = new Intent(SteelBattle.this, LoseScreen.class);
                    player.stop();
                    startActivity(i);
                    finish();
                }
