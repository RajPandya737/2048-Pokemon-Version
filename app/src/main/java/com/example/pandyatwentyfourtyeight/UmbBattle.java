...
public class UmbBattle extends AppCompatActivity {
    MediaPlayer player;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_umb_battle);
    ...
    public void throwball(View view) {
        ...
        Timer timer;
        if (a != 1) {
            t.setText("You have caught the Umbreon!");
            battle.setImageResource(R.drawable.s4after);

            timer = new Timer();
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    Intent i = new Intent(UmbBattle.this, GameScreen.class);
            ...
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    Intent i = new Intent(UmbBattle.this, LoseScreen.class);
                    player.stop();
                    startActivity(i);
                    finish();
                }
