...
public class SylveBattle extends AppCompatActivity {
    MediaPlayer player;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_sylve_battle);
    ...
    public void throwball(View view) {
        ...
        Timer timer;
        if (a != 1) {
            t.setText("You have caught the Sylveon!");
            battle.setImageResource(R.drawable.s6after);

            timer = new Timer();
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    Intent i = new Intent(SylveBattle.this, GameScreen.class);
            ...
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    Intent i = new Intent(SylveBattle.this, LoseScreen.class);
                    player.stop();
                    startActivity(i);
                    finish();
                }
